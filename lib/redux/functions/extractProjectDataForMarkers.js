...
const _getLocation = (project) => {
    ...
    if(firstLocation[1] && firstLocation[2]) {
        return {lat: parseFloat(firstLocation[1]), lng: parseFloat(firstLocation[2])};
    } else if (secondLocation[1] && secondLocation[2]) {
        return {lat: parseFloat(secondLocation[1]), lng: parseFloat(secondLocation[2])};
    } else {
        return null;
    }
};
